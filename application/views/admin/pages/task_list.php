						...
						if(!empty($info)){ $count = 1;
							...
							<tr>
							  ...
							  <td><?= ($row->task_hired == 1) ? '<i class="fas fa-handshake"></i>' : '' ?></td>
							   <td><?php 
							  $u_id= $row->user_task_id;
							  $hireDate= $this->db->get_where('task_hired',array('user_task_id'=>$u_id))->row();
							  $date=date_create($hireDate->hire_date);
							  echo date_format($date,"D d M Y");
							   ...
							   <td><?php 
							  $u_id= $row->user_task_id;
							  $username= $this->db->get_where('users',array('user_id'=>$u_id))->row();
							  echo $username->name;
							 

							   ?></td>
							  <td><?= ($row->task_status == 1) ? 'Competed' : 'Pendding' ?></td>
							</tr>
							<?php
							}
						}else{
