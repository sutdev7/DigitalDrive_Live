			...
			jQuery.ajax({
             ...
             async:false,
              success: function(data){
				 // alert(data);
				  if(data=='true'){
					  jQuery('#frmChangePassword').trigger("reset");
					  jQuery('.successpassword').html('Password changed successfully.');
				  }
				if(data=='wrong'){
					jQuery('.errors').html('Your old password is wrong.');
				}
				
				/* if(data=='postsuccess'){
				      jQuery('#user_post_textarea').val('');
				     jQuery('#overlay').fadeIn().delay(3000).fadeOut();
					
				setTimeout(function() {
					//	 jQuery('.loader_image').hide();
					 jQuery('.comment_post_using_ajax').load(location.href + ' .comment_post_using_ajax');
				  }, 3000); 
				 }*/
			  }		   
         });
    ...
    <div class="profile_top">
      <div class="container">
        <div class="row">
          ...
          <div class="col-lg-9" style="margin-top: -11%;">
            <div class="pro_info">
			<div class="successpassword"></div>
