...
if(!function_exists('count_message__view')){
    function count_message__view($user_id) {
		
		$CI =& get_instance();
		$CI->db->select('id');
		$CI->db->where('user_id_from',$user_id);
		$CI->db->where('is_read','N');
		$count = $CI->db->count_all_results('user_messages');
		return $count;
    }
}
...
/**
 * This function is used to get time ago string from current time
 *
