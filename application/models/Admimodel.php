...
class Admimodel extends CI_Model {
	...
	public function get_client_list($usertype = 0, $status = ""){
	...
	public function get_user_bank_info($user_id = ''){
 	...
 	public function get_user_portfolio_id($userid = ''){
...
public function get_user_tasko_id($userid = ''){
	...
	public function store_ticket($post_data = array()){

		$problem_ticket_no = $post_data['problem_ticket_no'];
		$email_to = $post_data['email_to'];
		$email_body = strip_tags($post_data['email_body']);
...
//echo "<pre>";print_r($inserdata);die;
	...
	function Update_read_status($usertype=0,$status=1){
		
		$this->db->set('admin_read', $status);
		$this->db->where('user_type', $usertype);
	...
	{
		$this->db->select('user_grievance.*, grievance.problem_type, grievance.problem_type_other, users.name, user_login.*');
		$this->db->from('user_grievance');
		$this->db->join('grievance', 'grievance.fid = user_grievance.grievance_id');
		$this->db->join('users', 'users.user_id = user_grievance.user_id');
		$this->db->join('user_login', 'user_login.user_id = user_grievance.user_id');
	...
	public function get_message_users() {
        $this->db->select('users.name,users.user_id, user_login.email, user_login.user_type , user_login.is_login, user_login.profile_id, CASE WHEN user_login.profile_image is null THEN "assets/img/no-image.png" ELSE CONCAT("uploads/user/profile_image/",user_login.profile_image) END profile_image ', FALSE);
		...
		{
		  $sql = "select * FROM user_messages where user_id_from = '" . $single->user_id . "' and user_id_to = '" . $this->session->userdata('user_id') . "' AND is_read='N'";
	...
	public function get_message_users_list() {
        $this->db->select('users.name,users.user_id, user_login.email, user_login.user_type , user_login.is_login, user_login.profile_id, CASE WHEN user_login.profile_image is null THEN "assets/img/no-image.png" ELSE CONCAT("uploads/user/profile_image/",user_login.profile_image) END profile_image ', FALSE);
        $this->db->from('user_messages');
        $this->db->join('users', 'users.user_id = user_messages.user_id_from', 'LEFT');
        $this->db->join('user_login', 'user_login.user_id = user_messages.user_id_from', 'LEFT');
	    ...
	    public function get_message_userss_model() {
		...
		if($type_id==5){
			$this->db->where('user_login.user_type', 5);
		}
        $this->db->group_end();
        $this->db->group_by('user_login.user_id');
        ...
        return $result;
		    ...
		    public function get_message_userss_outbox_model() {
			$type_id = $this->uri->segment(3); 
			$id='N4IND81M4L';
        $this->db->select('users.name,users.user_id, user_login.email, user_login.user_type , user_login.is_login, CASE WHEN user_login.profile_image is null THEN "assets/img/no-image.png" ELSE CONCAT("uploads/user/profile_image/",user_login.profile_image) END profile_image ', FALSE);
        $this->db->from('user_messages');
        $this->db->join('users', 'users.user_id = user_messages.user_id_to', 'LEFT');
        $this->db->join('user_login', 'user_login.user_id = user_messages.user_id_to', 'LEFT');
        $this->db->join('country', 'country.country_id = users.country', 'LEFT');
        $this->db->where('user_messages.user_id_from', $id);
        $this->db->group_start();
		if($type_id==4){
        $this->db->where('user_login.user_type', 4);
		} 
	...
	public function get_messages($user_id = "") {
        ...
        if ($user_id != "") {
           ...
           // $this->db->where('user_messages.user_id_to', $this->session->userdata('user_id'));
           // $this->db->or_where('user_messages.user_id_to', $user_id);
            //$this->db->group_start();
           // $this->db->or_where('user_login.user_type', 3);
           // $this->db->or_where('user_login.user_type', 4);
           // $this->db->or_where('user_login.user_type', 5);
            //$this->db->group_end();
            $this->db->group_by('user_messages.id');
			$this->db->order_by("user_messages.id", "asc");
            $query = $this->db->get();
          //echo $this->db->last_query();
            $result = $query->result();
			//dd($result);die;
	    ...
	    public function outbox_get_user_messages($user_id = "") {
        $result = array();
		$admin_id= $this->session->userdata('user_id'); 
		 $userID='PMPQMWGW76';
        if ($user_id != "") {
			 $outboxquery = "SELECT users.name, users.user_id, user_messages.message_content
FROM users
LEFT JOIN user_messages ON users.user_id = user_messages.user_id_to
WHERE user_messages.user_id_from='$admin_id' AND user_id_to='$user_id'"; 
$query   = $this->db->query($outboxquery);
$result = $query->result();
           /* $this->db->select('user_messages.*,users.name,users.user_id, user_login.email, user_login.user_type , user_login.is_login, CASE WHEN user_login.profile_image is null THEN "assets/img/no-image.png" ELSE CONCAT("uploads/user/profile_image/",user_login.profile_image) END profile_image ', FALSE);
            $this->db->from('user_messages');
            $this->db->join('users', 'users.user_id = user_messages.user_id_to', 'LEFT');
	...
	public function naluacer_detail_model_old(){
	...
	public function naluacer_detail_model(){
		$string=$this->uri->segment(3);
		$usertype=5;
		$this->db->select('users.*,user_login.*,user_login.status as active_status,country.name as country_name');
		$this->db->from('users');
