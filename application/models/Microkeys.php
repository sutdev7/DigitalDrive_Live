...
class Microkeys extends CI_Model {
    ...
    public function monthly_yearly_income_analytics($userID){
        $CI =& get_instance();
        $CI->load->model('Tasks');
    ...
    public function get_microkey_count_freelance($searchValue){
        //$user_id = $this->session->userdata('user_id');
        //$this->db->select('task_proposal.*, task.*, DATE_FORMAT(task.task_due_date, "%d/%m/%Y") as task_due_date, users.*');
        $this->db->select('microkey.*,users.*');
        $this->db->from('microkey');
   ...
   /*
	...
	public function add_microkey_proposal($postData = array()){
		...
		$insert = array(
			'microkey_job_id' => $this->input->post('microkey_id'),
			'user_id' => $this->session->userdata('user_id'),
		...
		$this->db->insert('microkey_proposal',$insert);
		$microkeyData = $this->get_microkey_job_info_by_id($this->input->post('microkey_id'));
		$notification_data = array(
			'micro_task_id' => $this->input->post('microkey_id'),
			'notification_from' => $this->session->userdata('user_id'),
			'notification_to' => $this->input->post('microkey_user_id'),
			'notification_title' =>  '<strong>'.'<a href='.base_url().'public-profile/'.$this->session->userdata('profile_id').'>'.$this->session->userdata('user_name').'</a></strong> has sent a proposal for the project <strong>'.$microkeyData->title.'</strong>',
			'notification_message' =>'',
			'notification_doc' => $date_of_creation,
			'isReadByClient' => 'Y'
		);

		$result_sub = $this->db->insert('micro_task_notification',$notification_data);
	...
	public function microkey_proposal_already_added(){
		$microkey_id = $this->input->post('microkey_id');
		$user_id = $this->session->userdata('user_id');
		$data = $this->db->select("count(*) as is_added")
		->from('microkey_proposal')
		->where('microkey_job_id', $microkey_id)
		->where('user_id', $user_id)
		->get()->row_array();
		return $data;
	}
	// modified on 23-10-2020
	public function check_is_inappropriate_microkey_job($microkeyjob_id = '', $user_id = ''){
				
		if($microkeyjob_id != ''){
			$this->db->select('*');
			$this->db->from('microkey_inappropriate');
			$this->db->where('microkey_inappropriate.user_id', $user_id);
			$this->db->where('microkey_inappropriate.microkey_id', $microkeyjob_id);
			$query = $this->db->get(); 
			if($query->num_rows() > 0){
	...
	// modified on 23-10-2020
	public function get_microkey_job_info_by_id($microkeyjob_id = ''){
        $this->db->select('microkey_client.*');
        $this->db->from('microkey_client');
        $this->db->where('microkey_client.id',$microkeyjob_id);
        $query = $this->db->get();
        if($query->num_rows() > 0){
            return $query->row();
        }else{
	...
	//modified on 23-10-2020
	public function save_inappropriate_microkey($microkey_id,$userID){
		$sql="UPDATE `microkey_inappropriate` SET `status` = 1 WHERE microkey_id = '$microkey_id' AND user_id='$userID';";
		$getInfo = $this->db->query($sql);
		if($this->db->affected_rows()==0){
			$date=date("Y-m-d H:i:s");
			$sql="INSERT INTO `microkey_inappropriate` (`microkey_id`,`user_id`,`created_on`)
				VALUES('$microkey_id','$userID','$date');";
			 $this->db->query($sql);
		}
	}
	//modified on 23-10-2020
	public function save_microkey_job($microkey_id,$userID){
		$sql="UPDATE `microkey_saved` SET `isActive` = 1 WHERE microkey_job_id = '$microkey_id' AND user_id='$userID';";
		$getInfo = $this->db->query($sql);
		if($this->db->affected_rows()==0){
			$date=date("Y-m-d H:i:s");
			$sql="INSERT INTO `microkey_saved` (`microkey_job_id`,`user_id`,`doc`)
				VALUES('$microkey_id','$userID','$date');";
			 $this->db->query($sql);
		}
	}
	// modified on 23-10-2020
	public function get_microkey_job_hire_info_by_jobid($microkeyjob_id = '',$return_type=''){
        $this->db->select('micro_key_freelancer_hire.*');
        $this->db->from('micro_key_freelancer_hire');
        $this->db->where('micro_key_freelancer_hire.mkfh_id',$microkeyjob_id);
        $query = $this->db->get();
        if($return_type == ''){
			return $query->num_rows();
	...
	// modified on 23-10-2020
	public function check_is_saved_micro_job($microkey_job_id = '', $user_id = ''){
				
		if($microkey_job_id != ''){
			$this->db->select('*');
			$this->db->from('microkey_saved');
			$this->db->where('microkey_saved.user_id', $user_id);
			$this->db->where('microkey_saved.microkey_job_id', $microkey_job_id);
			$query = $this->db->get(); 
			if($query->num_rows() > 0){
				return true;
			}else{
	...
	public function microkey_proposals($microkey_job_id = '',$return_type =''){
		if($microkey_job_id == ''){
			return 0;
		}else{
			$this->db->select('*');
			$this->db->from('microkey_proposal');
			$this->db->join('microkey_client', 'microkey_proposal.mkp_id = microkey_client.id');        
			$this->db->where('microkey_proposal.is_deleted', 0);      
			$this->db->where('microkey_proposal.microkey_job_id',$microkey_job_id);
			$query = $this->db->get();
