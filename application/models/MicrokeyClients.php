...
class MicrokeyClients extends CI_Model {
...
public function add_new_microkey_client($userID = null, $microkeyData = array()){
		//echo '<pre>'; print_r($userID); echo '<br/>'; print_r($microkeyData); die;
        if(empty($microkeyData))
        	return array('status' => FALSE, 'message' => 'invalid_data');
		 $CI =& get_instance();
        ...
        $date_of_creation = date("Y-m-d H:i:s");
         ...
         $data = array('title' => $microkeyData['microkey_client']['title'],
            'user_id' => $userID,
             'description' => $microkeyData['microkey_client']['description'],              
             'skills' => implode(",", $microkeyData['microkey_client']['skills']),
             'budget' => $microkeyData['microkey_client']['budget'], 
             'continent' => $microkeyData['microkey_client']['continent'], 
             'duration' => $microkeyData['microkey_client']['duration'], 
             'duration_type' => $microkeyData['microkey_client']['duration_type'],
             'country_id' => $microkeyData['microkey_client']['country'],
             'image' => $microkeyData['microkey_client']['image'],
			 'addon_file_url' => $microkeyData['microkey_client']['addon_file_url'],
             'created'=>$date_of_creation
             );
        // echo '<pre>'; print_r($data);die();

        $result = $this->db->insert('microkey_client',$data);  

        $insert_id = $this->db->insert_id(); 
		$project_name=$microkeyData['microkey_client']['title'];
		$skills=implode(",", $microkeyData['microkey_client']['skills']);
		$freelancers=$this->get_freelancer_ids_by_skill_set($skills);
		// $userInfo = $CI->Users->get_user_info_by_id($userID);
		$userData = $CI->Users->get_user_profile_info_by_id($userID);
		foreach($freelancers as $freelancer){
			
			$notification_data = array(
							'micro_task_id' => $insert_id,
							'notification_from' => $userID,
							'notification_to' => $freelancer->user_id,
							'notification_title' => '<strong> ' . '<a href='.base_url().'public-profile/'.$userData['basic_info']->profile_id.'>'. $userData['basic_info']->name.'</a></strong> ' .'Offered A Micro Key Project',
							'notification_message' => '<strong> ' . '<a href="'.base_url().'microkey-client-details/'.$insert_id.'">'.$project_name.'</a></strong> ',
	...
	public function get_freelancer_ids_by_skill_set($skill_sets=null){
		if($skill_sets==null) return array();
		
		$sql_matched_freelaner="SELECT DISTINCT user_area_of_interest.user_id
					FROM user_area_of_interest
					WHERE area_of_interest_id IN (".$skill_sets.")";
		$CI =& get_instance();
		$q=$CI->db->query($sql_matched_freelaner);
		$result = $q->result();
		return $result;
	}
public function update_existing_microkey_client($microkeyClientId = null, $microkeyData = array()){
		//echo '<pre>'; print_r($userID); echo '<br/>'; print_r($microkeyData); die;
        if(empty($microkeyData))
        	return array('status' => FALSE, 'message' => 'invalid_data');
        ...
        //$id_generator = $this->auth->generator(10);
        $date_of_creation = date("Y-m-d H:i:s");
		if(!empty($microkeyData['microkey_client']['image'])){
			 $data = array('title' => $microkeyData['microkey_client']['title'],
             'description' => $microkeyData['microkey_client']['description'],              
             'skills' => implode(",", $microkeyData['microkey_client']['skills']),
             'budget' => $microkeyData['microkey_client']['budget'], 
			...
			//echo '<pre>'; print_r($data);die();
			$this->db->where('id',$microkeyClientId);
			$result = $this->db->update('microkey_client',$data); 
		}else{
			 $data = array('title' => $microkeyData['microkey_client']['title'],
             'description' => $microkeyData['microkey_client']['description'],              
             'skills' => implode(",", $microkeyData['microkey_client']['skills']),
    ...
    public function get_microkey_count_ofclient($searchValue){
        $user_id = $this->session->userdata('user_id');
        //$this->db->select('task_proposal.*, task.*, DATE_FORMAT(task.task_due_date, "%d/%m/%Y") as task_due_date, users.*');
        $this->db->select('microkey_client.*,users.*');
        $this->db->from('microkey_client');
        //$this->db->join('task','task.task_id = task_proposal.task_id');
       $this->db->join('users','users.user_id = microkey_client.user_id');
        //$this->db->where('task.task_status', 1);
        //$this->db->where('task.task_is_ongoing', 0);         
        //$this->db->where('microkey.user_id',$user_id);
        $query = $this->db->get();
    ...
    public function get_microkey_data_ofclient($searchValue,$rowperpage,$rowno){
        ...
        //$this->db->where('task.task_status', 1);
        //$this->db->where('task.task_is_ongoing', 0);         
        //$this->db->where('microkey_client.user_id',$user_id);
        if($searchValue!=""){
            //$this->db->or_like('users.name',$searchValue,'both');
            //$this->db->like('task.task_name',$searchValue,'both');
        ...
        $this->db->order_by('microkey_client.created','desc');
        $this->db->limit($rowperpage,$rowno);
        $query = $this->db->get();
        // echo $this->db->last_query(); 
    //   echo '<pre>'; print_r($query->result_array()); die('model');
