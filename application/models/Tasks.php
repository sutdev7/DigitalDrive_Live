 ...
 public function project_detailsByType($type,$user_id, $limit = 0)
	...
	public function add_new_task($userID = null, $taskData = array()){
        ...
        $data = array(
            ...
            'milestone_type' => $taskData['task']['milestone_type'],
    ...
    public function get_hired_task_info_by_user_task_id($user_task_id = null){
        ...
        $this->db->select('task_hired.hired_id,task_hired.offer_id,task_hired.task_id,task_hired.proposal_id,task_hired.freelancer_id,task_proposal_milestone.milestone_title,task_proposal_milestone.milestone_end_date,task_proposal_milestone.milestone_agreed_budget,task.task_name AS hired_title, task.user_task_id');
        ...
        $this->db->where('task_hired.user_task_id', $user_task_id);
        $query = $this->db->get();
...
//        $query = $this->db->get();
 ...
 public function client_analytics_by_id($user_id){
    ...
    public function list_user_all_hired_tasks($userID = null, $limit = 10, $start = 1) {
        ...
        //$this->db->where('task.task_due_date > NOW()'); 
        $this->db->order_by('task_hired.hired_id','DESC'); 
    ...
    public function get_proposal_hired($task_id = '', $proposal_id =''){
    	
    	$this->db->select('*');
		$this->db->from('task_hired AS th');
		$this->db->where('th.task_id',$task_id);
		$this->db->where('th.proposal_id',$proposal_id);
        $query = $this->db->get();
       // echo $this->db->last_query(); exit;
        if($query->num_rows() > 0) {
            return $query->result_array();
