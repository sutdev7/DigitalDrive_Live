...
class Task extends CI_Controller {
    ...
    public function received_offers($taskID){
        $content = $this->ltask->received_offers_page($this->session->all_userdata(), $taskID);
        $data = array(
            'content' => $content,
            'title' => display('Received Offers :: Hire-n-Work'),
        );
        $this->template->full_customer_html_view($data);
    }

    public function view_received_offer($OffertaskID = null, $proposal_id = null)	{
	...
	public function browse_task($pageIndex=0){
		$content = $this->ltask->browse_task_page($pageIndex);
		$data = array(
			        'content' => $content,
			        'title' => display('Browse Task :: Hire-n-Work'),
		        );
	...
	public function download_attachment_proposal($filename = null) {
        ...
        $this->ltask->download_attachment_proposal($filename);
    }
