...
class Googlelogin extends CI_Controller {
	...
	public function login($type='')	{
		if(empty($type)){
			...
			$subscription_plan = $this->uri->segment(4);
		}
		...
		if(isset($_GET['code'])){
			$jha= $gClient->authenticate($_GET['code']);
		...
		if ($gClient->getAccessToken()) {
            ...
            $user_Gdata =  array($username,$email,$mobile,$imge_url,$name,$type);
            //echo"<pre>";print_r($user_Gdata);exit;	
            ...
            if ($result){
                ...
                if(isset($subscription_plan) && $subscription_plan !='1'){
                	redirect('subscription/payment/'.$subscription_plan.'/'.$resultG[0]['user_id'], 'refresh');
                }else{
