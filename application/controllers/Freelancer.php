...
class Freelancer extends CI_Controller {
	...
	public function details_projectFreelincer($id){

		$content = $this->lmicrokey->microkey_project_list_freelincer_page($id);
	...
	public function problem_ticket_details(){
    	$task['task_id'] = $this->uri->segment(2);
		$content = $this->lfreelancer->problem_ticket($task);		
	...
	public function save_problem_ticket(){
		...
		if(!empty($_POST) && is_array($_POST)){
			...
			//redirect('problem-ticket/'.$_POST['task_id'], 'refresh');
		}else{
			redirect('problem-ticket/'.$_POST['task_id'], 'refresh');
		}
		
		//echo"<pre>";print_r($task_id);exit;
		//$this->form_validation->set_rules('problem_id','Issue Type', 'required');
		//$this->form_validation->set_rules('task_id','Project Name', 'required');
		//$this->form_validation->set_rules('grievance_subject','Other Issue Type', 'required');
        //$this->form_validation->set_rules('grievance_content','Description','required');

        /*if($this->form_validation->run() == false){
			$this->lfreelancer->add_ticket($this->session->all_userdata());
		}
		redirect('problem-ticket/'.$task_id, 'refresh');*/
	}
