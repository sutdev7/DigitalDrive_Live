...
exit; */
	...
	public function ajax_search_top_freelancer() {
		...
		$freelancers = $CI->Users->get_freelancers_info();
		...
		foreach($freelancers as $key => $row){
			...
			$skills=$CI->Users->get_user_skill_list($row->user_id);
					...
					<div class="card gigs-task">
						...
						<div class="card-body position-relative">
							...
							<div class="gig-task-prof">
								...
								<img src="'.$user_profile_image.'" class="user-img">

								<p class="my-2 ">'.$row->name.' <span class="d-block small"></p>

								<p class="my-2 ">'.implode(",",$skillName).' <span class="d-block small"></p>

								<div class="row">

									<div class=" col  my-2 small "><span class="text-primary"><b>'.$row->job_complete_count.'</b></span> Projects Done</div>

									<div class=" col my-2 small "><span class="text-primary"><b>'.$row->total_coins.'</b></span> Coins</div>
						...
						<form name="frmMakeOffer" id="frmMakeOffer_'.$row->user_id.'" action="'.base_url().'make-an-offer" method="post">

							<input type="hidden" name="chkMakeOfferFreelancer" value="'.$row->user_id.'">

							<input type="hidden" name="fldSelectedFreelancer" value="'.$row->user_id.'">

							<button type="submit" class="btn btn-success m-auto">Hire</button>
	...
	public function ajax_search_freelancer() {
        ...
        if(!empty($userData) && is_array($userData)) {
            ...
            foreach($userData as $key => $row) {
                ...
                $output .= '<div class="anaaDiv"><div class="anaaDiv-top"> <span> <img id="userImage_'.$row['basic_info']->user_id.'" src="'.$user_profile_image.'" alt="'.$row['basic_info']->user_id.'" style="width:64px; height:64px;"> </span>
								...
								<form name="frmMakeOffer" id="frmMakeOffer_'.$row['basic_info']->user_id.'" action="'.base_url().'make-an-offer" method="post">
    ...
    public function client_portfolio_page(){
        ...
        $data['bankData']=$CI->Users->get_user_bankInfo($CI->session->userdata('user_id'));

        $data['user_login_data']=$CI->Users->get_user_ueser_login_id($CI->session->userdata('user_id'));         

        $data['user_info'][] = $userInfo;

        $data['portfolioData']= $portfolioData;

        $data['portfolioData']=$CI->Users->get_user_portfolio_id($CI->session->userdata('user_id'));
...
//        c_pr($userInfo);
        ...
        $AccountForm = $CI->parser->parse('user/edit-profile', $data, true);
...
//        }
	...
	public function public_profile_page_admin($user_id,$user_type){	
            ...
            $user_detail=	$CI->Users->get_user_profile_info_by_id_details($user_id);
           	$user_public_id= $user_detail->profile_id;
          ...
          // echo '<pre>'; print_r($userLoginData); print_r($top_freelancers); die;
            ...
            if (!empty($userLoginData)) {
               ...
               if ($user_type == 4) {
                    ...
                    $view_page = 'user/public_profile_freelancer_admin';
		...
		if($user_type == 4){
			...
			$user_details= $this->db->get_where('user_login',array('user_id'=>$user_id))->row();
			 ...
			 echo base_url().'freelancer-dashboard/'.$user_details->username;die();

			$username=$this->session->set_userdata($user_details->usname);

			redirect('freelancer-dashboard/'.$username,'refresh');
...
//                    }
	...
	public function updateUserProfile($userInfo = null){
		...
		if(!empty($userInfo)) {
           ...
           //echo "<pre>";print_r($data);die;
		    ...
		    $result = $CI->Users->update_profile($data, $userInfo['user_id']);
		    ...
		    if($result) {
                ...
                $CI->session->set_flashdata('msg', '<div class="alert alert-success text-center">You have successfully fill the form , please, wait for 3 to 4 days so we can validate your date.</div>');
    ...
    public function insertPortfolioData($userInfo = null){
     ...
     public function updatePortfolioData($userInfo = null){
        ...
        if(!empty($userInfo)) {
            ...
            $result = $CI->Users->update_portfolio($data,$editId,$imagename, $userInfo['user_id']);
    ...
    public function messages($user_id_to = '') {
        ...
        $data = $this->get_messages($user_id_to);

		$userData = $CI->Users->get_user_profile_info_by_id($user_id_to);

        $data['user_id_to'] = $user_id_to;

		$data['otheruserInfo'] = $userData;

		$data['login_session'] = $CI->Users->get_login_session($user_id_to);

        $AccountForm = $CI->parser->parse('user/messages', $data, true);

        //echo '<pre>'; print_r($AccountForm); echo '</pre>';exit;
    ...
    public function get_messages($user_id_to = '') {
        ...
        if($_SESSION['user_type'] > 1) // restrict user for admin temp by vb
...
//            echo '<pre>';  print_r($frndlist);echo '</pre>';die;
        ...
        //echo '<pre>';print_r($msghistory);die();
        ...
        //Get Other User Is Login and his/her Last Seen

        if($user_id_to) {

            $otherUserData = $CI->Users->get_user_profile_info_by_id($user_id_to);

            $otherUserLogin = $otherUserData['basic_info']->is_login;

            $lastSeen = $CI->Messages->lastSeen($user_id_to);

            $data['lastSeen'] = ($otherUserLogin == 1) ? 'Online' : $lastSeen;
    ...
    /** function to delete message **/
...

		{
		  ...
		  $CI->db->query("UPDATE user_messages SET deleted='1' WHERE user_id_from='" . $user_id . "' and id='" . $message_id . "'");

		}



   /** function to update user msg **/

    public function updateMsgUserData($message_id){
        ...
        if(!empty($data)) {
            ...
            $result = $CI->Messages->update_user_message($data, $userIdTo,$message_id);
   ...
   public function addMsgUserDataAjax($code){
        ...
        if(!empty($data)) {
            ...
            if(!empty($_FILES) && is_array($_FILES)){
                ...
                for($i = 0; $i<count($_FILES['uploadFiles']['name']); $i++) {
                    ...
                    if(move_uploaded_file($sourcePath,$targetPath)) {
						...
						$data['original_attachement'][] = array('fname' => $path);
            ...
            $result = $CI->Messages->add_user_message($data, $userIdTo,$code);
   ...
   public function updateMsgUserDataAjax($message_id){
	...
	public function get_unloaded_messages($user_id_to = '',$message_id) {
        ...
        if ($user_id_to != '') { 
            ...
            $msghistory = $CI->Messages->get_unloaded_message_data_by_id($user_id_to,$message_id);
