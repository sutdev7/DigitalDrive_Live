	...
	public function past_microkey_page_1($postData = array()){
	 ...
	 public function microkey_project_list_client_page($id){
       ...
       // $CI =& get_instance();
        ...
        $MicrokeyDataok  = $CI->parser->parse('freelancer/micro_procect_list',$microkeyData,true);
...
#Abhishek
	...
	public function microkey_proposal_already_added() {
		$CI =& get_instance();		
		$CI->load->model('Microkeys');
		$data = $CI->Microkeys->microkey_proposal_already_added();
		return $data;		
	}
	//modified on 23-10-2020
	public function submit_microkey_proposal(){
		...
		$CI->load->model('Microkeys');
		$CI->load->model('Users');
		$isExist = $CI->Microkeys->microkey_proposal_already_added();
			if($isExist['is_added']){
				...
				$result=$CI->Microkeys->add_microkey_proposal($postData);
				$microkeyData = $CI->Microkeys->get_microkey_job_info_by_id( $CI->input->post('microkey_id'));
				if($result){
					$CI->session->set_flashdata('msg', '<div class="alert alert-success text-center">Your proposal for   '.$microkeyData->title.' has been submitted.</div>');
					redirect('microkey-list-clientpanel','refresh');
				}else{
	...
	//modified on 23-10-2020
	public function add_microkey_proposal_layout(){
		$data=array();
		$CI =& get_instance(); 
		$CI->load->model("Microkeys");
		$CI->load->model('Skills');
		$CI->load->model('Freelancers');
		$microkey_user_id=$CI->input->post('microkey_user_id');
		$microkey_id=$CI->input->post('microkey_id');
		$userId=$CI->session->userdata('user_id');
		// get freelancer connection 
		$getConnects = $CI->Freelancers->get_user_basic_info($userId);
		if(!empty($getConnects)){
			$data['connection'] = $getConnects['total_connects'];
		}else{
			$data['connection'] = 0;
		}
		$microkeyData = $CI->Microkeys->get_microkey_job_info_by_id($microkey_id);
		$clientId=$microkeyData->user_id;
		$required_skills = $microkeyData->skills;
		$created_on = $microkeyData->created;
		$duration_type = $microkeyData->duration_type;
		$duration = $microkeyData->duration;
		$SkillsName = $CI->Skills->get_all_skill_info_by_id($required_skills);
		
		 $arrFileName = explode('_', $microkeyData->image);
                $file_typ_img = get_file_ext($microkeyData->image);
                $task_attachments[] = array('file_ext_type' => $file_typ_img,  'file_name' => $microkeyData->image, 'file_display_name' => end($arrFileName));
		$data['task_title']=$microkeyData->title;
		$data['description']=$microkeyData->description;
		$data['budget']=$microkeyData->budget;
		$data['skill_name']=$SkillsName;
		$data['created_on']=$created_on;
		$data['task_attachments']=$task_attachments;
		
		// echo '<pre>'; print_r($data);die();
		$AccountForm = $CI->parser->parse('freelancer/microkey-proposal',$data,true);
		return $AccountForm;
	}
	//modified on 23-10-2020
	public function save_inappropriate_microkey(){
		$CI =& get_instance(); 
		$CI->load->model("Microkeys");
		$microkey_id=$CI->input->post('microkeyId');
		$userID=$CI->session->userdata('user_id');
		//echo $microkey_id." ".$userID;die;
		$microkeyData = $CI->Microkeys->save_inappropriate_microkey($microkey_id,$userID);
		echo "saved";
	}
	//modified on 23-10-2020
	public function save_microkey_job(){
		...
		$userID=$CI->session->userdata('user_id');
		//echo $microkey_id." ".$userID;die;
		$microkeyData = $CI->Microkeys->save_microkey_job($microkey_id,$userID);
		echo "saved";
	}
	//modified on 23-10-2020
	public function get_microkey_details_by_id($microkey_id){
		$CI =& get_instance(); 
		$CI->load->model('Users'); 
		$CI->load->model('Skills');
		$CI->load->model('Microkeys');
		...
		//$req_skill_name = implode(",",$skillNames);
		
		//check is saved
		$savedcheck = $CI->Microkeys->check_is_saved_micro_job($microkey_id, $userId);
		if($savedcheck){
		...
		// check inappropriate
		$savedinappropriate = $CI->Microkeys->check_is_inappropriate_microkey_job($microkey_id, $userId);
		...
		$data['proposal_count'] = $CI->Microkeys->microkey_proposals($microkey_id,'');
		$data['task_freelancer_hire'] = $CI->Microkeys->get_microkey_job_hire_info_by_jobid($microkey_id,'');
		 $datetime1 = strtotime($created_on);
			...
			{
				$tduration= $duration.' Day';
			}
			else if($duration_type=='Monthly')
			{
				$tduration= $duration.' Month';
			}
			else if($duration_type=='Yearly')
			{
				$tduration= $duration.' Year';
			}
			$user_details = $CI->Users->get_user_profile_info_by_id($clientId);
			$user_status = $CI->Users->get_user_info_by_id($clientId);
			// job creator info
			$creator_info = $CI->Freelancers->get_user_basic_info($clientId);
			$creator_post_info = $CI->Freelancers->get_client_post_count_info($clientId);
			// echo '<pre>'; print_r($user_details);die();
			//echo $microkeyData->continent;
			$continent = $CI->Continent->get_continent_by_id($microkeyData->continent);
            $country = $CI->Countries->get_country_by_id($microkeyData->country_id);
			$data['task_title']=$microkeyData->title;
			$data['description']=$microkeyData->description;
			$data['created_on']=$created_on;
			...
			$data['skill_name']=$SkillsName;
			$data['client_name']=$user_details['basic_info']->name;
			$data['client_id']=$clientId;
			$data['microkey_job_id']=$microkey_id;
			...
			//echo '<pre>'; print_r($microkeyData);die();
			$AccountForm = $CI->parser->parse('freelancer/micro_key_job_details',$data,true);
			return $AccountForm;
	...
	public function add_microkey_client_post($userInfo = null) {
            ...
            if (!empty($submitData)) {
                ...
                $table_data['microkey_client'] = array('title' => $submitData['title'],
                    ...
                    'addon_file_url' => $submitData['addon_file_url'],
		...
		public function edit_microkey_client_post($id = null,$userInfo = null) {
            $CI = & get_instance();
            $CI->load->model('Tasks');
            $CI->load->model('MicrokeyClients');
            $CI->load->model('Admin_notification');
            ...
            $table_data = array();
            $submitData = $CI->input->post();
            // echo '<pre>'.$id; print_r($submitData);
            // echo '<pre>'; print_r($userInfo);die();
            if (!empty($submitData)) {
                $table_data['microkey_client'] = array('title' => $submitData['title'],
                    'description' => $submitData['description'],
                    'budget' => $submitData['budget'],
                    'continent' => $submitData['fldSelContinent'],
                    'duration' => $submitData['duration'],
                    'duration_type' => $submitData['duration_type'],
                    'country' => $submitData['fldSelCountry'],
					'addon_file_url' => $submitData['addon_file_url']
				...
				$table_data['microkey_client']['image']="";
                if (!empty($_FILES) && is_array($_FILES)) {
                ...
                $result = $CI->MicrokeyClients->update_existing_microkey_client($id, $table_data);
                if ($result) {
                    $notification_data['msg'] = "A Microkey edited by " . $userInfo['user_name'];
                    $CI->Admin_notification->add_notification($notification_data);
                    $CI->session->set_flashdata('msg', '<div class="alert alert-success text-center">Microkey modified successfully.</div>');
                    //return array('status' =>  TRUE, 'message' => 'Task data save successfully.');            	
                } else {
                    $CI->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Unable to modified Microkey.</div>');
                    //return array('status' =>  FALSE, 'message' => 'Unable to save task data.');
                }
            } else {
                $CI->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Unable to modified Microkey.</div>');
