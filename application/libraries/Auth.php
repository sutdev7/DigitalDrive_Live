...
class Auth {
    ...
    public function sendthankyouEmail($receiver , $type=null){
		$CI =& get_instance();

        $from = $CI->config->item('smtp_user');    //senders email address
        $subject = 'Thank you for registering with us.';  //email subject
        
        //sending confirmEmail($receiver) function calling link to the user, inside message body
        $message = 'Dear User,<br><br> Thank you so much for registering with us. Feel free to contact our support team if any issue occurs with your account. We will be glad to give you support.<br><br>
        <a href=\'' . base_url().'account/verify/'.base64_encode($receiver).'\'>' . base_url().'account/verify/'. base64_encode($receiver) .'</a><br><br>Thanks';
        
        $data['receiver']= $receiver;
        $body = $CI->load->view('email/thankyou_email_template.php',$data,TRUE);
        $CI->load->library('email');
        //send email
        $CI->email->set_newline("\r\n");
		$CI->email->from('info@hirenwork.com');
