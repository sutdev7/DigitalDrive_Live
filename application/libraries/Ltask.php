...
//echo '<pre>'; print_r($details['task_attachments']);die;
    ...
    public function microkey_client_details_page_details($taskID = null, $userInfo = null) {
...
//echo '<pre>'; print_r($details['task_attachments']);die;
	...
	public function add_new_job($userInfo = null){
        ...
        if($submitData["milestone_subtype"] == "yes" && $submitData["milestone_type"] == "pay_by_milestone"){
          $milestone_type = "milestone";

        }else if($submitData["milestone_subtype"] == "no" && $submitData["milestone_type"] == "pay_by_milestone"){
           $milestone_type = "fixed";

        } else if($submitData["milestone_type"] == "fixed"){
            $milestone_type = "fixed";
...
//        echo "<pre>";print_r($task); exit();
        foreach ($task as $details) {
            ...
            //print_r($details['task_hired']); 
            //echo count($details['task_hired']); exit();

            if (!empty($details['task_hired']) ) {

               //print_r($details['task_hired']); exit();
                foreach ($details['task_hired'] as $freelancer_hired) {
...
//echo '<pre>'; print_r($details['task_attachments']);die;
        ...
        //echo '<pre>'; print_r($data['top_freelancers']); die;
        ...
        // echo '<pre>'; print_r( $data['task_info']); exit;
        ...
        foreach($jobs as $job) {
            ...
            if(!empty($jobInfo)) {
                ...
                $amoutperhr_details = "";
                 ...
                 $is_hired = $CI->Tasks->get_proposal_hired($job->task_id,$job->proposal_id);
                 if(!empty($is_hired)){
                    $hired_id[] = $is_hired[0]['hired_id'];
                 } 
