...
class Lhire {
	...
	public function add_direct_hire($post_data = null){
        ...
        $user_task_id = $post_data['fldJobTitle'];

		$return = $CI->Hires->hire_direct_data_insert($post_data);
		if($return['status'] ==true){
            $milestone_id = $return['milestone_id'];
	        if(trim($CI->input->post('deposit')) == 'deposit_fund_now'){
				redirect(base_url().'hire/step-2/'.$milestone_id, 'refresh');
			}else{
	...
	public function hire_step2($userInfo = null){
		...
		if(!empty($get_info)){
			$taskArr[] = array('task_id' => $get_info[0]->task_id,'usertaskid_milestoneid'=> $str, 'user_task_id' => $get_info[0]->user_task_id, 'proposal_id'=> $get_info[0]->proposal_id, 'task_name' => $get_info[0]->hired_title, 'task_total_budget' => $totalsum,'freelancer_id'=>$get_info[0]->freelancer_id );
	...
	public function direct_hire_freelancers($FR_id=''){
		...
		$data = $selectedFreelancer = $arrFreelancer = $arrJobs = $arrCountry = $arrContinent = $arrSkills = [];
        ...
        if(isset($FR_id) && $FR_id !=''){
            ...
            $freelancer_id = $post_data['fldSelectedFreelancer'];    
        }
        //$freelancer_id = $post_data['fldSelectedFreelancer'];
        $jobs = $CI->Hires->task_data_for_directHire();
        ...
        if(!empty($post_data)) {
			...
			}else if(isset($freelancer_id) && $freelancer_id !=''){
                $selectedFreelancer[] = $freelancer_id;
            } else {
            	$selectedFreelancer[] = $post_data['chkMakeOfferFreelancer'];
            }
            ...
            foreach($freelancers_list as $row) {
				...
				if(!empty($user_status)){
                    if($user_status->total_coins>=0){ 
                        $total_positive_coins='+ '.$user_status->total_coins;
                    }else{ 
                        $total_positive_coins=$user_status->total_coins;
                    }
	...
	public function hire_freelancers_rehire($freelancer_id){
		...
		$jobs = $CI->Hires->get_task_data_by_user($freelancer_id);
	...
	public function view_contract($hired_id = 0, $freelancer_id =0){
		...
		if(!empty($contract_info)){
		   ...
		   // echo "<pre>"; print_r($contract_info);die;
			...
			$data = array(
                ...
                'hired_proposal' => $contract_info['hired_proposal'],

				'freelancer_id' => $contract_info['contract_details']['freelancer_id'],

				'task_name' => isset($task_data->task_name) ? $task_data->task_name : $contract_info['contract_details']['hired_title'],
                'task_is_complete' => isset($task_data->task_is_complete) ? $task_data->task_is_complete : 0,
		...
		if(isset($data['contract_milestone_info'][0])) {
            $data['contract_milestone_info'][0]['task_name'] = $data['task_name'];
        }
	...
	public function ajax_change_milestone_date(){
		...
		$data['updateTable'] = 'task_proposal_milestone';
		if(!empty($data)){
	...
	public function ajax_add_new_milestone(){
		...
		if(!empty($data)){
			...
			if($return['status']==TRUE){
                $paynow = $return['msg'];
				$data = array('status' => 1, 'message' => 'successful','paynow'=>$paynow);

			}else{
                $data = array('status' => 0, 'message' => 'error','paynow'=>'');
			}
	...
	public function close_contract_page(){
		...
		if(!empty($contractInfo)){
            ...
            if($freelancerInfo['basic_info']['total_coins']>=0){ $positivecoin='+ '.$freelancerInfo['basic_info']['total_coins'];
            }else{ 
                $positivecoin = $freelancerInfo['basic_info']['total_coins'];
            }
        ...
        if(isset($data['task_id']) && $data['task_id'] !=''){
            $data['task_details'] = $CI->Freelancers->get_task_details($data['task_id']);
        }else{
            $data['task_details'] = $CI->Freelancers->get_task_details();
        }
