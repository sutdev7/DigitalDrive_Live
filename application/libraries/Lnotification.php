...
$data['micro_notifications']=$this->notification_details_micro_project($user_id);
	...
	public function notification_details_micro_project($userInfo = null){
        ...
        $notifications = $CI->Notifications->get_micro_project_notification_data($userInfo);  
        ...
        foreach($notifications as $notification) {
            ...
            $arrNotifications[] = array( 'notification_details' => $notification->notification_title, 'notification_message' => $notification->notification_title.$notification->notification_message, 'hours' => $hours, 'notification_send_by' => $userData['basic_info']->name, 'user_image' => $user_profile_image);
	...
	public function take_action_condition($notirow_id = 0, $action_id = 0){
		...
		// get notification type
			...
			if(!empty($notification_info)){
                ...
                $notification_details = $notification_info->notification_details;

				$createrinfo = $CI->Notifications->get_task_info_with_creater($task_id);
                //echo "<pre>"; print_r($createrinfo);       
            ...
            $task_total_budget = $task_info->task_total_budget;
            $milestone_type = $task_info->milestone_type;
           ...
           // echo "<pre>"; print_r($masterInfo); exit();      
			...
			if($action_id == 12){
                ...
                $updateProposal= array(
                    ...
                    'terms_amount_max'=>$task_total_budget,
                    'terms_amount_max'=>$task_total_budget,
                    'cover_letter'=>"ACCEPT AN OFFER",
                ...
                $proposal_id = $CI->Notifications->insert_notification('task_proposal',$updateProposal);
                if($proposal_id>0){

                    $offerMilsstone= array(
                        'proposal_id'=>$proposal_id,
                        'milestone_title'=> $title,
                        'milestone_end_date'=> date('Y-m-d H:i:s'),                    
                        'milestone_agreed_budget'=>$task_total_budget,
                        ...
                        'milestone_created_by'=>$notification_to,
                    );
                    $updatmilesstone = $CI->Notifications->insert_notification('task_proposal_milestone',$offerMilsstone);
                }
                 ...
                 $insertHire = array(
                    ...
                    'freelancer_id' => $notification_to,
                    'hired_title'   => $title,
                    'hired_details' => $notification_details,
                    'agreed_budget' => $task_total_budget,
                    'agreed_term' => $milestone_type,
                    'term_amount' =>$task_total_budget,
                    'is_direct_hiring' => 1,
                    'hire_date' => 1,
                    'hired_is_open' => 1,
                    'hire_is_completed' => 1,
                    'hired_status' => 1,
                    'hired_is_deleted' => 0,
                    'offer_send_by' =>$notification_from,
                    'hire_date' => date('Y-m-d H:i:s'),
                    'hired_doc' => date('Y-m-d H:i:s')
                );
                $ssts = $CI->Notifications->insert_notification('task_hired',$insertHire);
			...
			// task completed approved
				...
				*/
				...
				// redirect('hired-job-details/'.$task_info->user_task_id,'refresh');
				 ...
				 redirect('givereview/'.$notification_from,'refresh'); exit;
				...
				$CI->Notifications->insert_notification('task_notification',$insert);
				redirect('notification','refresh'); exit;
			}
